...
// Store last signature per token to avoid duplicates
...
export async function GET(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const poll = async () => {
        try {
          const lastSig = lastSignatures.get(tokenAddress);
          const response = await helius.getTransactionHistory(tokenAddress, lastSig, 10);
          
          if (response && response.length > 0) {
            // Update last signature
            lastSignatures.set(tokenAddress, response[0].signature);
